...
class Node:
    ...
    def start(self):
        ...
        for mgr in self.mgrs.values():
            mgr.start()
        # run tests after mgrs have started
        self.run()
        try:
        ...
        while not self.is_stopped and self.pm.state is State.STARTED:
            if not self.pm.recv_queue[0].empty():
                ...
                if new_stat['data']['genesisHash'] == self.status['genesisHash']:
                    ...
                    # self is newer --> send blocks     # ugly ugly code QQQQQQ
...
class dbMgr(gevent.Greenlet):
    ...
    def run(self):
        ...
        while not self.is_stopped and self.pm.state is State.STARTED:
            # get broadcasted block --> add block to db
            ...
            # get broadcasted transaction --> add trans to pendingtransactions
            ...
            if not self.pm.recv_queue[4].empty():
                ...
                #TODO: 
                # if still missing blocks, send another request msg_03, else update status --> where to store list of requested blocks?
    ...
    def run(self):
        ...
        while not self.is_stopped and self.pm.state is State.STARTED:
            ...
            if not self.pm.recv_queue[8].empty():
                ...
                consensus['hash'] = int(consensus['hash'],16)
                Database.createBlock([consensus], self.db)          # broadcast??
