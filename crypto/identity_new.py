...
'''
    ...
    Usage notes:
        ...
        ** Signing & Verifying **
        ...
        s = identity.priv2pemlong(priv)                         # create signing key from private key
        pub = identity.priv2pub(priv)                           # create public key from private key
        v = identity.pub2vkey(pub)                              # create verifying key from public key
        
        start_time = time.time()
        signed = identity.signdata(data=data, signkey=s)        # sign data with signingkey, returns signature string
        end_time = time.time()
        print("Sign, time elapsed: %f" %(end_time-start_time))
        
        start_time = time.time()
        result = identity.verifydata(sigdata=signed, origdata=data, verkey=v)   # verify data with verifykey, signature, and original data
        end_time = time.time()
...
class identity():
    ...
    def signdata(cls, priv=None, wif=None, signkey=None, data=None):
        ...
        (r, s) = fecdsa.sign(data, signkey, fcurve.secp256k1)
        return str(r) + str(s)
...
print("Result: %s, time elapsed: %f" %(result, end_time-start_time))
'''
Not available anymore. Do not test this part.
